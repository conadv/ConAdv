...
flags.DEFINE_integer(
    ...
    help=('size parameter of DropBlock. Will not be used if dropblock_groups '
          'is empty.'))
flags.DEFINE_integer('attack_iter', help=('Adversarial attack iteration'),
                                    default=1)
flags.DEFINE_float('attack_epsilon', help=('Adversarial attack maximal perturbation'),
                                    default=9.0)
flags.DEFINE_float('attack_step_size', help=('Adversarial attack step size'),
                default=9.0)
...
flags.DEFINE_float(
    'warm_lr', default=43.0,
    help=('warm up learning rate.')
        )

flags.DEFINE_integer(
    'warm_epoch', default=41,
    help=('warm up epoch in the training stage.'))
...
class PGDAttacker:
    ...
    def __init__(self, num_iter, epsilon, step_size, prob_start_from_clean=0.0):
        ...
        self.vars_params = tf.trainable_variables()
    ...
    def attack(self, image_clean, label, model_func):
        ...
        def one_step_attack(adv):
            ...
            trainable_variables_original = tf.trainable_variables()
            i = 1
            original_params = []
            assignments = []
            for param in trainable_variables:
                vars_m = tf.get_variable(
                    name="adam_m" + str(i),
                    shape=param.shape.as_list(),
                    dtype=tf.float32,
                    trainable=False,
                    initializer=tf.zeros_initializer())
                original_params.append(vars_m)
                vars_m_1 = tf.multiply(vars_m, 1.0)
                assignments.append(param.assign(vars_m_1))
                i = i+1
            print("###################################", i)
            
            if not self.USE_FP16:
                logits = model_func(adv, mixbn=True, adv=True)
            else:
                adv16 = tf.cast(adv, tf.float32)
                # with custom_getter_scope(fp16_getter):
                with tf.tpu.bfloat16_scope():
                    logits = model_func(adv16, mixbn=True, adv=True)
                    logits = tf.cast(logits, tf.float32)
            # Note we don't add any summaries here when creating losses, because
            # summaries don't work in conditionals.
            # pdate_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            ...
            if not self.USE_FP16:
                ...
                g = g / 128.
            # def count():
            # [trainable_variables] = tf.cond(tf.train.get_global_step() > 1, lambda: [tf.trainable_variables()], lambda: [trainable_variables_original])
                # self.vars_params = tf.cond(tf.train.get_global_step() > 2, lambda: self.vars_params, lambda: tf.trainable_variables())
            
            
            trainable = tf.trainable_variables()

            for i in range(len(trainable_variables)):
                trainable[i].assign(trainable_variables_original[i])
                original_params[i].assign(trainable_variables_original[i])

            # return 1
            # self.va = tf.cond(tf.train.get_global_step() > 2, lambda: count(), lambda: 2)
            ...
            """
            # adv = adv - tf.sign(g) * self.step_size
            adv = tf.clip_by_value(adv - tf.sign(g) * self.step_size, lower_bound, upper_bound)
            return adv, tf.group(*assignments, name="parameter")
        ...
        """
        init_start = tf.random_uniform(tf.shape(image_clean), minval=-self.epsilon,
                                       maxval=self.epsilon)
        start_from_noise_index = tf.cast(tf.greater(
            tf.random_uniform(shape=[]), self.prob_start_from_clean), tf.float32)
        start_adv = tf.cast(tf.cast(image_clean, tf.float32) + start_from_noise_index * init_start, tf.bfloat16)
        ...
        with tf.name_scope('attack_loop'):
            ...
            # def first_step_logit(mixbn, adv):
            # with tf.tpu.bfloat16_scope():
            #     logits_original = model_func(start_adv, mixbn=True, adv=True )
            #     logits_adver = models_func(start_adv, mixbn=True, adv=False)
                # return logits
            # logits = tf.cond(tf.train.get_global_step() > 1,lambda: first_step_logit(mixbn=True, adv=True), lambda: first_step_logit(mixbn=False, adv=False))
            # self.vars_params = tf.trainable_variables()
            for i in range(self.num_iter):
                adv_final, train_op = one_step_attack(adv_final)
            """
            adv_final = tf.while_loop(
                lambda _: True,
                one_step_attack,
                # if not self.USE_XLA else
            ...
            """
        print("adv_final: ", adv_final)
        return adv_final, target_label, train_op
...
# attacker = PGDAttacker(1, 7.0, 7.0, prob_start_from_clean=0.2)
...
def resnet_model_fn(features, labels, mode, params):
  ...
  print("#################features.shape: ", features.shape)
  # features = tf.concat((features[:,:,:,:3], features[:,:,:,3:]), axis=0)
  ...
  features_input = tf.reshape(features, (2, features.shape[0]//2, features.shape[1], features.shape[2], features.shape[3]))
  features = features_input[0]
  features_augment = features_input[1]
  # attacker = PGDAttacker(params['attack_iter'], params['attack_epsilon'], params['attack_step_size'], prob_start_from_clean=0.2 if not mode == tf.estimator.ModeKeys.EVAL else 0.0)

  # This nested function allows us to avoid duplicating the logic which
  # builds the network, for different values of --precision.
  def build_network(features, mixbn=True, adv=False):
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
      ...
      network = resnet_model.resnet(
        ...
        mixbn=mixbn, adv=adv)
      return network(
        inputs=features, is_training=(mode == tf.estimator.ModeKeys.TRAIN), mixbn=mixbn, adv=adv)

  eval_features = features
  eval_labels = labels


  input_feature = tf.concat([features, features_augment, features_augment], axis=0)
  with tf.tpu.bfloat16_scope():
      logits = build_network(input_feature, mixbn=True, adv=False)
  """
  i = 1
  trainable_variables = tf.trainable_variables()
  ...
  attacker = PGDAttacker(params['attack_iter'], params['attack_epsilon'], params['attack_step_size'], prob_start_from_clean=0.2 if not mode == tf.estimator.ModeKeys.EVAL else 0.0)
  # print("#################################", i)
  adv_image, target_label, train_op_total = attacker.attack(features, labels, build_network)
  adv_image = tf.stop_gradient(adv_image, name='adv_training_sample')
  print("features_1: ", features)
  features = tf.concat([features[:96], features_augment[:96], adv_image[:96]], axis=0)
  print("features_2: ", features)
  labels = tf.concat([labels[:96], labels[:96], labels[:96]], axis=0)
  """
  i = 1
  with tf.variable_scope('V1', reuse=tf.AUTO_REUSE):
    for param in trainable_variables:
  ...
  if params['precision'] == 'bfloat16':
    with tf.tpu.bfloat16_scope():
      logits = build_network(features, mixbn=True, adv=False)
      if mode == tf.estimator.ModeKeys.EVAL:
          eval_logits = build_network(eval_features, mixbn=False, adv=False)
    logits = tf.cast(logits, tf.float32)
    if mode == tf.estimator.ModeKeys.EVAL:
        eval_logits = tf.cast(eval_logits, tf.float32)
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    predictions = {
        'classes': tf.argmax(eval_logits, axis=1),
        'probabilities': tf.nn.softmax(eval_logits, name='softmax_tensor')
    }
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    # the train operation.
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    # update_ops = tf.group([update_ops, train_op_total])
    with tf.control_dependencies(update_ops):
    ...
    if not params['skip_host_call']:
      def host_call_fn(gs, loss, lr, ce):
        ...
        for v in tf.trainable_variables():
            # if('batch_normalization/gamma' in v.name):
            #     bn_gamma = tf.reduce_mean(v, keep_dims=False)
            # if('batch_normalization/beta' in v.name):
            #     bn_beta = tf.reduce_mean(v, keep_dims=False)
            if('mixbn2/batch_normalization/gamma' in v.name):
                mixbn_gamma = tf.reduce_mean(v, keep_dims=False)
            elif('mixbn2/batch_normalization/beta' in v.name):
                mixbn_beta = tf.reduce_mean(v, keep_dims=False)
            elif('mixbnaugment2/batch_normalization/gamma' in v.name):
                mixbnaug_gamma = tf.reduce_mean(v, keep_dims=False)
            elif('mixbnaugment2/batch_normalization/beta' in v.name):
                mixbnaug_beta = tf.reduce_mean(v, keep_dims=False)
            elif('batch_normalization/gamma' in v.name and 'mixbn' not in v.name):
                bn_gamma = tf.reduce_mean(v, keep_dims=False)
            elif('batch_normalization/beta' in v.name and 'mixbn' not in v.name):
                bn_beta = tf.reduce_mean(v, keep_dims=False)
        with tf2.summary.create_file_writer(
          ...
          with tf2.summary.record_if(True):
            ...
            tf2.summary.scalar('bn_gamma', bn_gamma, step=gs)
            tf2.summary.scalar('bn_beta', bn_beta, step=gs)
            tf2.summary.scalar('mixbn_gamma', mixbn_gamma, step=gs)
            tf2.summary.scalar('mixbn_beta', mixbn_beta, step=gs)
            tf2.summary.scalar('mixbnaug_gamma', mixbnaug_gamma, step=gs)
            tf2.summary.scalar('mixbnaug_beta', mixbnaug_beta, step=gs)
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    eval_metrics = (metric_fn, [eval_labels, eval_logits])
