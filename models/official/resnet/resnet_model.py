...
def norm_activation(
    ...
    bn_momentum=MOVING_AVERAGE_DECAY, mixbn=True,adv=False):
  ...
  if layer == LAYER_BN_RELU:
    return batch_norm_relu(
        ...
        bn_momentum=bn_momentum, mixbn=mixbn,adv=adv)
...
def batch_norm_relu(inputs, is_training, relu=True, init_zero=False,
  ...
  if (mixbn):
      global L
      L += 1
      # with tf.variable_scope("mixbn{}".format(L)):
      ...
      if (adv):
          ...
          inputs = tf.reshape(inputs, [3, inputs.shape[0] // 3, inputs.shape[1], inputs.shape[2], inputs.shape[3]])
          inputs_1 = tf.layers.batch_normalization(
              inputs=inputs[0],
              axis=axis,
              momentum=BATCH_NORM_DECAY,
              epsilon=BATCH_NORM_EPSILON,
              ...
              gamma_initializer=gamma_initializer)
          # if(mixbn):
          with tf.variable_scope("mixbnaugment{}".format(L)):
              inputs_2 = tf.layers.batch_normalization(
                      inputs=inputs[1],
                      axis=axis,
                      momentum=BATCH_NORM_DECAY,
                      epsilon=BATCH_NORM_EPSILON,
          ...
          #   L += 1
          with tf.variable_scope("mixbn{}".format(L)):
              inputs_3 = tf.layers.batch_normalization(
                  inputs=inputs[2],
                  axis=axis,
                  momentum=BATCH_NORM_DECAY,
                  epsilon=BATCH_NORM_EPSILON,
                  ...
                  fused=True,
                  gamma_initializer=gamma_initializer)
          inputs = tf.concat([inputs_1, inputs_2, inputs_3], axis=0)
          print("L: ", L)
      if (L == 54):
          L = 1
      # inputs = tf.reshape(inputs,[-1, inputs.shape[0], inputs.shape[1], inputs.shape[2]])
      # print("inputs.shape_: ", inputs.shape)
      # inputs = inputs_1
  else:
      # inputs = inputs_1
...
def residual_block(inputs, filters, is_training, strides,
                   ...
                   drop_connect_rate=None, bn_momentum=MOVING_AVERAGE_DECAY, mixbn=True, adv=False):
  ...
  if pre_activation:
    inputs = norm_activation(inputs, is_training, data_format=data_format,
                             layer=norm_act_layer, bn_momentum=bn_momentum, mixbn=mixbn, adv=adv)
  if use_projection:
  ...
  if pre_activation:
    ...
    inputs = norm_activation(
        ...
        data_format=data_format, layer=norm_act_layer, bn_momentum=bn_momentum, mixbn=mixbn, adv=adv)
...
def resnet_generator(block_fn,
  ...
  def model(inputs, is_training, mixbn=True, adv=False):
...
resnet_v1 = functools.partial(resnet, pre_activation=False, mixbn=True, adv=False)
resnet_v2 = functools.partial(resnet, pre_activation=True, mixbn=True, adv=False)
resnet_v1_generator = functools.partial(resnet_generator, pre_activation=False, mixbn=True, adv=False)
resnet_v2_generator = functools.partial(resnet_generator, pre_activation=True, mixbn=True, adv=False)
